...
def main(config):
    ...
    explainer = GraphSVX(dataset, model, device, 
    ...
    for i, data in enumerate(tqdm(dataset[test_indices])):
        idx = test_indices[i]
        explained_example_path = os.path.join(explanation_saving_path, f'example_{idx}.pt')
        if not IS_FRESH and os.path.isfile(explained_example_path):
            ...
            node_scores = explainer.explain(data,
        ...
        if config.save_plot:
            ...
            from utils import scores2coalition, evaluate_coalition, fidelity_normalize_and_harmonic_mean, to_networkx
    ...
    metrics = evaluate_scores_list(explainer, 
