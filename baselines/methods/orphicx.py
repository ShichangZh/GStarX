...
class Orphicx(ExplainerBase):
    ...
    '''
    def __init__(self, vgae, classifier, device, num_causal_factors, explain_graph=True):
    ...
    def forward(self, data, sparse_batch,
                **kwargs)\
        ...
        hard_edge_masks = [self.control_sparsity(edge_mask, sparsity=sparsity).sigmoid().to(self.device)
...
class GraphSampler(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.dense_x, self.dense_mask = to_dense_batch(batch_dataset.x, batch_dataset.batch)
        ...
        for graph_idx in tqdm(graph_idxs):
            ...
            pos_weight = float(adj.shape[0] * adj.shape[0] - adj.sum()) / max(float(adj.sum()), 1e-3)
...
def dense_to_sparse_batch(x, adj, mask):
    data_list = [Data(x=x[i][mask[i]], edge_index=dense_to_sparse(adj[i][mask[i]][:, mask[i]])[0]) for i in range(x.shape[0])]
...
def gae_loss(preds, labels, mu, logvar, n_nodes, norm, pos_weight):
    bce = F.binary_cross_entropy_with_logits(preds.flatten(1).T, labels.flatten(1).T,pos_weight=pos_weight,reduction='none').mean(0)
    cost = norm * bce
...
#     parser.add_argument('--Nbeta', type=int, default=100, help='Number of monte-carlo samples per noncausal factor.')
    ...
    parser.add_argument('--Nalpha', type=int, default=15, help='Number of monte-carlo samples per causal factor.')
    parser.add_argument('--Nbeta', type=int, default=50, help='Number of monte-carlo samples per noncausal factor.')
...
#     parser.add_argument('--one_hot_label_dim', type=int, default=100, help='dimensions used for one_hot_label.')
    parser.add_argument('--eval_epoch', type=int, default=1, help='How many epochs for one eval.')
...
def eval_model(args, orphicx, dataset, device, criterion, ceparams):
    ...
    with torch.no_grad():
        for data in dataset:
            sparse_batch = dense_to_sparse_batch(data['feat'], data['sub_adj'], data['feat_mask']).to(device)
            for k in data:
            ...
            masked_recovered_sparse_batch = dense_to_sparse_batch(data['feat'], masked_recovered_adj_hard, data['feat_mask'])
            recovered_logits = orphicx.model(sparse_batch.x, masked_recovered_sparse_batch.edge_index)
            ...
            masked_alpha_sparse_batch = dense_to_sparse_batch(data['feat'], masked_alpha_adj_hard, data['feat_mask'])
            alpha_logits = orphicx.model(sparse_batch.x, masked_alpha_sparse_batch.edge_index)
            ...
            masked_beta_sparse_batch = dense_to_sparse_batch(data['feat'], masked_beta_adj_hard, data['feat_mask'])
            beta_logits = orphicx.model(sparse_batch.x, masked_beta_sparse_batch.edge_index)
            
            causal_loss = []
            beta_info = []
            for idx in random.sample(range(0, data['feat'].shape[0]), args.NX):                 
                _causal_loss, _ = causaleffect.joint_uncond(ceparams, orphicx.explainer.dc, orphicx.model, data['sub_adj'][idx], data['feat'][idx], act=torch.sigmoid, device=device)
                _beta_info, _ = causaleffect.beta_info_flow(ceparams, orphicx.explainer.dc, orphicx.model, data['sub_adj'][idx], data['feat'][idx], act=torch.sigmoid, device=device)
                ...
                for A_idx in random.sample(range(0, data['feat'].shape[0]), args.NA-1):
                    ...
                    _causal_loss, _ = causaleffect.joint_uncond(ceparams, orphicx.explainer.dc, orphicx.model, perm_adj, data['feat'][idx], act=torch.sigmoid, device=device)
                    _beta_info, _ = causaleffect.beta_info_flow(ceparams, orphicx.explainer.dc, orphicx.model, perm_adj, data['feat'][idx], act=torch.sigmoid, device=device)
                    causal_loss += [_causal_loss]
                    beta_info += [_beta_info]
        ...
        alpha_sparsity = masked_alpha_adj.mean((1,2))/ org_adjs.mean((1,2)).clamp(min=1e-3)
