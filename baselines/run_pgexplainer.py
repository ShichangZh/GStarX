...
def pipeline(config):
    ...
    pgexplainer_saving_path = os.path.join(explanation_saving_path, f'{explainer_name}.pth')
    ...
    for i, data in enumerate(tqdm(dataset[test_indices])):
        ...
        if config.save_plot:
            ...
            coalition = hard_edge_masks2coalition(data, hard_edge_masks, config.models.param.add_self_loop)
