...
from baselines.methods.orphicx import gaeloss, get_orphicx_default_args, dense_to_sparse_batch, eval_model, GraphSampler
IS_FRESH = False
# IS_FRESH = True
...
def pipeline(config):
    ...
    orphicx_data_saving_path = os.path.join(config.datasets.dataset_root, config.datasets.dataset_name,
                                            'orphicx_processed_dataset.pt')
    if os.path.isfile(orphicx_data_saving_path):
        logger.info("Loading processed dataset")
        processed_dataset = torch.load(orphicx_data_saving_path, map_location=torch.device('cpu'))    
    else:
    ...
    vgae = VGAE3MLP(
        dataset.num_node_features + args.one_hot_label_dim, args.encoder_hidden1, args.encoder_hidden1,
        args.encoder_output, args.decoder_hidden1, args.decoder_hidden2,
        args.K, args.dropout
    ).to(device)

    orphicx = Orphicx(vgae, model, device, num_causal_factors=args.K, explain_graph=config.models.param.graph_classification)
    ...
    if not IS_FRESH and os.path.isfile(orphicx_saving_path):
        ...
        for epoch in tqdm(range(1, args.epoch+1)):
            ...
            for data in train_dataloader:  
                ...
                alpha_logit = orphicx.model(sparse_batch.x, masked_alpha_sparse_batch.edge_index)
                ...
                if args.coef_kl:
                    klloss = args.coef_kl * F.kl_div(F.log_softmax(alpha_logit,dim=1), org_probs)
                else:
                    klloss = 0
                if args.coef_size:
                ...
                loss.backward()
                nn.utils.clip_grad_norm_(orphicx.explainer.parameters(), args.max_grad_norm)
                optimizer.step()
                train_losses += [[nll_loss, causal_loss, klloss, size_loss]]
                
            if epoch % args.eval_epoch == 0:
                val_loss = eval_model(args, orphicx, val_dataloader, device, criterion, ceparams)
                patient -= 1
                ...
                logger.info(f'train_loss: {np.array(loss.item()).round(5)}, val_loss: {np.array(val_loss).round(5)}')
    ...
    for i, data in enumerate(tqdm(test_dataloader)):
        ...
        edge_masks, hard_edge_masks, related_preds = orphicx(data, sparse_batch,
        ...
        if config.save_plot:
            ...
            plot_utils.plot(to_networkx(sparse_data), coalition, 
                            x=sparse_data.x, 
                            words=words,
