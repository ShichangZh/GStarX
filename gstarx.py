...
class GStarX(object):
    ...
    def explain(self, data, superadditive_ext=True, 
                sample_method='khop', num_samples=10, k=3):
        '''
        Args:
        ...
        char_func = get_char_func(self.model, target_class, self.payoff_type, self.payoff_avg)
        if data.num_nodes < self.max_sample_size:
            ...
            while not counts.all() or i < num_samples:
                sampled_nodes, sampled_data, sampled_adj = sample_subgraph(data, self.max_sample_size, sample_method, i, k, adj)
                sampled_scores = self.compute_scores(sampled_data, sampled_adj, char_func, superadditive_ext)
    ...
    def compute_scores(self, data, adj, char_func, superadditive_ext=True):
        ...
        v = get_coalition_payoffs(data, coalitions, char_func, self.subgraph_building_func)
